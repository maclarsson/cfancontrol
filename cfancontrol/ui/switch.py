...
class Switch(QAbstractButton):
    ...
    def set_colors(self, accent: QColor):
        ...
        palette.setColor(palette.Highlight, accent)
    ...
    def paintEvent(self, event):  # pylint: disable=invalid-name, unused-argument
        ...
        text_opacity = 1.0
        if self.isEnabled():
            track_brush: QBrush = self._track_color[self.isChecked()]
            thumb_brush: QBrush = self._thumb_color[self.isChecked()]
            text_color: QColor = self._text_color[self.isChecked()]
        else:
            track_opacity *= 0.8
            track_brush: QBrush = self.palette().shadow()
            thumb_brush: QBrush = self.palette().mid()
            text_color: QColor = self.palette().shadow().color()
